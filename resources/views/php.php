...
            z-index: 1050; /* تأكد من أنه أعلى من كل شيء */
        }
        
...
        /* ******* التعديل الأهم هنا لمرونة الـ Hero Image ******* */
        .hotel-hero {
            position: relative;
            /* استخدم ارتفاعًا مرنًا يعتمد على عرض النافذة */
            height: 60vh; /* 60% من ارتفاع منطقة العرض */
            min-height: 400px; /* الحد الأدنى للارتفاع */
            max-height: 700px; /* الحد الأقصى للارتفاع */
...
        /* **************************************************** */
        ...
        /* الإعدادات الأساسية لجميع الصور داخل Hero */
        .hotel-image-base {
            position: absolute; /* ضروري لوضعها فوق بعضها البعض */
            top: 0;
            /* right: 0; لإلغاء هذا لضبط موضع كل صورة على حدة */
            width: 100%;
            height: 100%;
            object-fit: cover;
        ...
        /* الصورة الخلفية (التي ستكون ذات اوباسيتي أقل وفي الجانب الأيسر) */
        .hotel-image-back {
            right: auto;
            left: 2%; /* على اليسار قليلاً */
            opacity: 0.7; /* شفافية أقل */
            transform: scale(0.98); /* تصغير بسيط لإعطاء تأثير الطبقات */
            z-index: 1; /* يجب أن تكون تحت الصورة الأمامية */
        }
        
        /* الصورة الأمامية (التي كانت hotel-image-full) */
        .hotel-image-full {
            right: 0; /* موقعها على اليمين (في اتجاه RTL) */
            width: 100%;
            height: 100%;
            object-fit: cover;
            /* هذا سيضمن أن الصورة تملأ الحاوية دون تشوه، حتى لو كانت طويلة، وسيتم قص الأطراف */
            transition: transform 0.5s ease;
            z-index: 2; /* تأكد من أنها فوق الخلفية */
        }
...
        }
        ...
        .hotel-hero:hover .hotel-image-back {
            transform: scale(1.01); /* تأثير حركة خفيف عليها هي الأخرى */
            opacity: 0.8;
        }
...
            }
            ...
            .hotel-image-back {
                left: 3%; /* زيادة في التباعد لتعويض الحجم الأصغر */
            }
            /* ********************************************* */
...
            .gallery-img { height: 180px; }
            .hotel-image-back { display: none; } /* إخفاء الصورة الخلفية على الشاشات الصغيرة جداً لعدم الحاجة إليها */
        }
    </style>
...
        {{-- قسم صورة البطل (Hero Image) --}}
        <div class="hotel-hero" data-aos="zoom-in">
            
            {{-- الصورة الخلفية (ذات الاوباسيتي الأقل وفي الجانب الأيسر) --}}
            <img src="{{ asset($hotel->image) }}" class="hotel-image-base hotel-image-back" alt="{{ $hotel->name }} - خلفية">

            {{-- الصورة الأمامية (الأصلية) --}}
            <img src="{{ asset($hotel->image) }}" class="hotel-image-base hotel-image-full" alt="{{ $hotel->name }}">
...
                        $roomName = $i == 1 ? 'جناح ملكي (سرير مزدوج كبير)' : ($i == 2 ? 'غرفة مزدوجة فاخرة' : 'غرفة عائلية سعة ' . $i . ' أسرّة');
                    @endphp
